...
| originTenantInfo | [crossTenantResource](crosstenantresource.md) | Contains origin and synchronization information about a synchronized user from a source Azure AD tenant. This information is only populated for users that are synchronized using Azure AD cross-tenant synchronization. Nullable. <br><br>Returned only on `$select`. Supports `$filter` (`eq`). |
